...
public class LoginAdmin extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        button_register.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent next = new Intent(getApplicationContext(),RegisterAdmin.class);
                startActivity(next);
            }
        });
                        ...
                        if (dataSnapshot.exists()) {
                            for (DataSnapshot customer: dataSnapshot.getChildren()) {
                                if(editText_password.getText().toString().equals(customer.child("password").getValue().toString())){
                                    customerLogin = 1;
                                    emailAdmin = editText_email.getText().toString();
                                    Intent next = new Intent(getApplicationContext(), DashboardAdmin.class);
                                    startActivity(next);
                                }else{
